...
function Characters() {
  ...
  const filter = Array.isArray(name)
    ? undefined
    : {
        name
      };
  const { data, loading, fetchMore, networkStatus } = useGetCharactersQuery({
    ...
    variables: !Array.isArray(name) ? { filter } : undefined,
